...
@MainActor
final class DebugDataLoader {
    static let shared = DebugDataLoader()
    private init() {}
    
    func loadDemoData(into context: ModelContext) async throws {
        // Clear existing data
    ...
    private func clearExistingData(context: ModelContext) async throws {
        ...
        try await deleteAllEntities(of: CardSchemaV1.StereoCard.self, from: context)
        try await deleteAllEntities(of: TitleSchemaV1.Title.self, from: context)
        try await deleteAllEntities(of: AuthorSchemaV1.Author.self, from: context)
        try await deleteAllEntities(of: SubjectSchemaV1.Subject.self, from: context)
        try await deleteAllEntities(of: DateSchemaV1.Date.self, from: context)
        try await deleteAllEntities(of: CropSchemaV1.Crop.self, from: context)
        try await deleteAllEntities(of: CollectionSchemaV1.Collection.self, from: context)
    ...
    // Copy over the entity creation methods from PreviewDataManager
